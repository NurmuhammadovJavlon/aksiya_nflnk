  ...
  try {
    ...
    if (dealerId) {
      orders = await GetAllInvalidOrders(
        ...
        ctx.wizard.state.order.dealerId
      );
    } else {
      orders = await GetAllInvalidOrders(
        ctx.wizard.state.order.orderPage,
...
startStep.hears(match("OperatorPanel.unSeenOrdersbtn"), async (ctx) => {
  try {
    ...
    ctx.wizard.state.order.dealerId = operator.dealerId;
...
sendOrdersStep.action("cancel", async (ctx) => {
  try {
    const MainMenu = await generateOperatorPanelKeys(ctx);
